...
class Trainer:
    """Класс, управляющий процессом обучения нейронной сети с использованием генетического алгоритма.

       Этот класс отвечает за управление процессом тренировки, включая создание среды,
       генетического алгоритма, оценку популяции, сохранение результатов и визуализацию процесса.

       Атрибуты:
           env (Environment): Среда, в которой происходит обучение агентов.
           ga (GeneticAlgorithm): Генетический алгоритм для эволюции популяции.
           best_fitness (float): Лучшая фитнес-оценка за все время.
           visualization_frequency (int): Частота визуализации (каждое n-е поколение).
           results_dir (str): Директория для сохранения результатов тренировки.
    """
