...
class Game:
    """
        Класс для игры "Self-Parking Car". Этот класс управляет всей логикой игры, включая обработку событий, обновление состояния и визуализацию.

        Атрибуты:
            WIDTH (int): Ширина игрового экрана.
            HEIGHT (int): Высота игрового экрана.
            screen (pygame.Surface): Экран для отображения игры.
            WHITE (tuple): Цвет фона экрана.
            BLACK (tuple): Цвет объектов.
            BLUE (tuple): Цвет для выделения объектов.
            RED (tuple): Цвет для отображения ошибок.
            GREEN (tuple): Цвет для положительных результатов (например, успешная парковка).
            PINK (tuple): Цвет для пользовательских препятствий.
            env (Environment): Объект среды, в котором происходит игра.
            visualization_mode (bool): Режим визуализации игры (если True, то игра отображается с использованием нейросети, если False — управление вручную).
            brain (Brain or None): Объект, представляющий нейросеть (если есть).
            custom_rectangles (list): Список пользовательских препятствий, которые могут быть добавлены на игровое поле.
            font (pygame.font.Font): Шрифт для отрисовки текста на экране.
            clock (pygame.time.Clock): Объект для контроля FPS.
            running (bool): Флаг, указывающий на состояние игры (работает ли она или завершена).

        """

    def __init__(self, visualization_mode=True, load_genome_path=None):
        """
            Инициализация игры, настройка экрана, создание среды и настройка параметров визуализации.

            Параметры:
                visualization_mode (bool): Если True, игра будет отображать визуализацию с использованием нейросети. Если False, управление будет ручным.
                load_genome_path (str, optional): Путь к файлу с геномом нейросети для загрузки. Если указано, то нейросеть будет загружена и использоваться в игре.

            Инициализирует:
                pygame (pygame): Инициализация библиотеки Pygame для работы с графикой.
                WIDTH (int): Ширина окна игры (800 пикселей).
                HEIGHT (int): Высота окна игры (600 пикселей).
                screen (pygame.Surface): Объект для отображения игры.
                WHITE, BLACK, BLUE, RED, GREEN, PINK (tuple): Цвета для отрисовки элементов.
                env (Environment): Объект среды, в которой происходит игра, отвечающий за состояние игры, парковочное место и барьеры.
                visualization_mode (bool): Флаг, определяющий, будет ли визуализация в игре.
                brain (Brain or None): Нейросеть, использующаяся для управления машиной в режиме визуализации. Если не указан путь к геному, остается пустой.
                custom_rectangles (list): Список пользовательских прямоугольников-препятствий.
                ...
                running (bool): Флаг, указывающий, продолжается ли игра (True) или завершена (False).
        """
